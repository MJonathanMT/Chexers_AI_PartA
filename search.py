...
def distance_fill(dist_dict, adj_dict, pos, dist):
    dist += 1
    if dist_dict[pos] < 0:
        dist_dict[pos] = dist
    ...
    for next_pos in adj_dict[pos]:
        distance_fill(dist_dict, adj_dict, next_pos, dist)
    return 0


def get_moves(dist_dict, adj_dict, pieces):
    best_moves = {}
    for piece in pieces:
        movement = []
        shortest_dist = 0
...
def play_game(board_dict, dist_dict, adj_dict, blockers, pieces, goals, colour):
    while pieces:
        ...
        # move normally
        if not exit_move:
            best_moves = get_moves(dist_dict, adj_dict, pieces)
...
def main():
    ...
    print_board(board_dict, 'Starting State of the Board', True)
